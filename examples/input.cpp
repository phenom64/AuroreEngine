...
class simple_phase : public dreamrender::phase, public input::keyboard_handler {
    public:
        ...
        void render(int frame, vk::Semaphore imageAvailable, vk::Semaphore renderFinished, vk::Fence fence) override {
            ...
            gui.draw_image_sized(texture, x-0.25f, y-0.25f, gui.frame_size.width/2, gui.frame_size.height/2);
        ...
        void key_down(sdl::Keysym key) override {
            if(key.sym == sdl::KeyCode::SDLK_UP) {
                y -= 0.01;
            } else if(key.sym == sdl::KeyCode::SDLK_DOWN) {
                y += 0.01;
            } else if(key.sym == sdl::KeyCode::SDLK_LEFT) {
                x -= 0.01;
            } else if(key.sym == sdl::KeyCode::SDLK_RIGHT) {
                x += 0.01;
            }
        }
    private:
...
int main() {
    ...
    window.init();
    auto* phase = new simple_phase(&window);
    window.set_phase(phase, phase);
    window.loop();
}
